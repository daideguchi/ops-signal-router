...
import { buildLocalRoute, shouldRouteLocally } from './local-route.js';
...
export async function handleAppMentioned({ client, context, event, logger, say, sayStream, setStatus }) {
  try {
    ...
    if (!cleanedText) {
      await say({
        text: 'Send me an ops signal, deadline notice, receipt, or account warning and I will route it into an action card.',
        thread_ts: threadTs,
      });
      return;
    }

    if (shouldRouteLocally(cleanedText)) {
      await setStatus({
        status: 'Routing signal...',
        loading_messages: ['Checking deadline, stopline, and next action...'],
      });
      const routed = buildLocalRoute(cleanedText);
      await say({
        text: routed.text,
    ...
    await setStatus({
      ...
      loading_messages: [
        'Reading the signal\u2026',
        'Checking deadlines and stoplines\u2026',
        'Building the action card\u2026',
